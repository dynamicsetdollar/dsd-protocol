...
describe('Comptroller', function () {
    ...
    describe('mintToAccount', function () {
        ...
        describe('bootstrapping', function () {
            describe('on single call', function () {
                beforeEach(async function () {
                    await this.comptroller.mintToAccountE(
                        userAddress,
                        new BN(100)
                    )
                })
                ...
                it('mints new Dollar tokens', async function () {
                    ...
                    ).to.be.bignumber.equal(new BN(10100))
                    expect(
                ...
                it('doesnt update total debt', async function () {
                    expect(
                        await this.comptroller.totalDebt()
                    ).to.be.bignumber.equal(new BN(0))
                })
            })
            ...
            describe('multiple calls', function () {
                ...
                it('mints new Dollar tokens', async function () {
                    expect(
                        await this.dollar.totalSupply()
                    ).to.be.bignumber.equal(new BN(10300))
                    expect(
                        await this.dollar.balanceOf(this.comptroller.address)
                    ).to.be.bignumber.equal(new BN(0))
                    expect(
    ...
    describe('burnFromAccount', function () {
        ...
        describe('on single call', function () {
            beforeEach(async function () {
                await this.comptroller.burnFromAccountE(
                    userAddress,
                    new BN(100)
                )
            })
        ...
        describe('call when not enough debt', function () {
            ...
            it('reverts', async function () {
                await expectRevert(
                    this.comptroller.burnFromAccountE(userAddress, new BN(200)),
                    'not enough outstanding debt'
                )
            })
        })
    })
    ...
    describe('redeemToAccount', function () {
        beforeEach(async function () {
            await this.comptroller.mintToE(
        ...
        describe('on single call', function () {
            ...
            it('updates total redeemable', async function () {
                expect(
                    await this.comptroller.totalRedeemable()
                ).to.be.bignumber.equal(new BN(200))
            })
        })
    ...
    describe('increaseDebt', function () {
        ...
        describe('increase past supply', function () {
            ...
            it('updates total debt', async function () {
                expect(
                    await this.comptroller.totalDebt()
                ).to.be.bignumber.equal(new BN(350))
            })
        })
    ...
    describe('decreaseDebt', function () {
        ...
        describe('on single call', function () {
            ...
            it('updates total debt', async function () {
                expect(
                    await this.comptroller.totalDebt()
                ).to.be.bignumber.equal(new BN(250))
            })
        })
        ...
        describe('multiple calls', function () {
            ...
            it('updates total debt', async function () {
                expect(
                    await this.comptroller.totalDebt()
                ).to.be.bignumber.equal(new BN(50))
            })
        })
        ...
        describe('decrease past supply', function () {
            it('reverts', async function () {
                await expectRevert(
                    this.comptroller.decreaseDebtE(new BN(400)),
                    'not enough debt'
                )
            })
        })
