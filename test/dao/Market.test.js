    ...
    describe("when redeeming cdsd for DSD", function () {
      ...
      describe("partially redeeming", function () {
        ...
        it("emits CDSDRedeemed event", async function () {
          const event = await expectEvent.inTransaction(this.txHash, MockMarket, "CDSDRedeemed", {
            account: userAddress,
          });
  ...
  describe("step", function () {
    ...
    describe("on call with expiration", function () {
      ...
      it("emits CouponExpiration event", async function () {
        ...
        expect(event.args.couponsExpired).to.be.bignumber.equal(new BN(100000));
    ...
    describe("with bonded", function () {
      ...
      describe("reclaimed all debt and some bonded", function () {
        ...
        it("emits CouponExpiration event", async function () {
          ...
          expect(event.args.epoch).to.be.bignumber.equal(new BN(2));
          expect(event.args.couponsExpired).to.be.bignumber.equal(new BN(50000));
          expect(event.args.lessDebt).to.be.bignumber.equal(new BN(0));
