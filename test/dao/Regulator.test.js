...
describe('Regulator', function () {
    ...
    beforeEach(async function () {
        ...
        this.regulator = await MockRegulator.new(
            this.oracle.address,
            poolAddress,
        ...
        describe('up regulation', function () {
                ...
                describe('on step', function () {
                    ...
                    it('mints new Dollar tokens', async function () {
                        ...
                        expect(
                            ...
                            poolIncentive(this.expectedReward)
                        )
                    })

                    it('updates totals', async function () {
                        expect(
                            await this.regulator.totalStaged()
                        ...
                        expect(
                            await this.regulator.totalSupply()
                        ).to.be.bignumber.equal(new BN(0))
                        expect(
                            await this.regulator.totalCoupons()
                        ).to.be.bignumber.equal(new BN(0))
                        expect(
                    ...
                    it('emits SupplyIncrease event', async function () {
                        ...
                        expect(event.args.epoch).to.be.bignumber.equal(
                            new BN(7)
                        )
                        expect(event.args.price).to.be.bignumber.equal(
                            new BN(115).mul(new BN(10).pow(new BN(16)))
                        )
            ...
            describe('(1) - refresh redeemable at specified ratio', function () {
                beforeEach(async function () {
                    ...
                    await this.regulator.incrementBalanceOfCouponsE(
                        userAddress,
                        1,
                        new BN(100000)
                    )
        ...
        describe('(3) - above limit but below coupon limit', function () {
            ...
            describe('on step', function () {
                ...
                it('updates totals', async function () {
                    ...
                    expect(
                        await this.regulator.totalRedeemable()
                    ).to.be.bignumber.equal(new BN(this.expectedRewardCoupons))
                })
                ...
                it('emits SupplyIncrease event', async function () {
                    ...
                    expect(event.args.epoch).to.be.bignumber.equal(new BN(7))
                    expect(event.args.price).to.be.bignumber.equal(
                        new BN(105).mul(new BN(10).pow(new BN(16)))
                    )
                    expect(event.args.newRedeemable).to.be.bignumber.equal(
                        new BN(this.expectedRewardCoupons)
                    )
            ...
            describe('under limit', function () {
                ...
                describe('on step', function () {
                    ...
                    it('updates totals', async function () {
                        ...
                        expect(
                            await this.regulator.totalDebt()
                        ).to.be.bignumber.equal(new BN(this.expectedDebt))
                        expect(
                            await this.regulator.totalSupply()
                        ).to.be.bignumber.equal(new BN(0))
                        expect(
                    ...
                    it('emits SupplyDecrease event', async function () {
                        ...
                        expect(event.args.price).to.be.bignumber.equal(
                            new BN(85).mul(new BN(10).pow(new BN(16)))
                        )
                        expect(event.args.newDebt).to.be.bignumber.equal(
                            new BN(this.expectedDebt)
                        )
                    })
                })
            })
            ...
            describe('without debt', function () {
                ...
                describe('on step', function () {
                    ...
                    it('emits SupplyDecrease event', async function () {
                        ...
                        expect(event.args.price).to.be.bignumber.equal(
                            new BN(99).mul(new BN(10).pow(new BN(16)))
                        )
                        expect(event.args.newDebt).to.be.bignumber.equal(
                            new BN(this.expectedDebt)
                        )
                    ...
                    it('updates totals', async function () {
                        it('updates totals', async function () {
                            ...
                            expect(
                                ...
                                new BN(100000).add(new BN(this.expectedDebt))
                            )
                            expect(
                                await this.regulator.totalSupply()
                            ).to.be.bignumber.equal(new BN(0))
            ...
            describe('with debt over default but under coupon limit', function () {
                ...
                describe('on step', function () {
                    ...
                    it('emits SupplyDecrease event', async function () {
                        ...
                        expect(event.args.price).to.be.bignumber.equal(
                            new BN(95).mul(new BN(10).pow(new BN(16)))
                        )
                        expect(event.args.newDebt).to.be.bignumber.equal(
                            new BN(this.expectedDebt)
                        )
